...
// Os dados de cada indice
typedef struct datanode {
  int id;
} DataNode;

// struct que armazena os dados da lista
typedef struct node {
  DataNode data;
  struct node* next;
} Node;

// struct que armazena a lista
typedef struct list {
  int size;
  Node* head;
} List;
...
// IMPLEMENTAÇÕES
List* createList() {
  List * list = (List*) malloc(sizeof(List)); // Cria a lista no momento da execução
...
Node* showNode(List* list, int index) {
  if(index >= 0 && index < list->size) { // Valida a posição informada
...
int showIndex(List* list, Node* node) {
  if(node != NULL) { // Valida a existencia do node
    Node* pointer = list->head; // Recupera o node da cabeça
    int index = 0; // Cria uma posição inicial

    while(pointer != NULL && pointer != node) { // Percorre todos os nodes
...
void destroy(List* list, int index) {
  if(index == 0) {
    ...
    if(nodeASerExcluido != NULL) { // Verifica a existencia do node
...
void insert(List* list, DataNode data, int index) {
  if(index == 0) 
    ...
    if(nodeAtual != NULL) { // Verifica a existencia do node indicado
...
void xchgNodes(List* list, Node* nodeA, Node* nodeB) {
  ...
  if(indexA > indexB) { // Verifica se o nodeA é maior que o nodeB. O nodeA deve ser anterior ao nodeB na lista
  ...
  if(nodeA == list->head) { // Verifica se o nodeA esta na cabeça da lista
